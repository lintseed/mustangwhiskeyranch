...
/*
 * Copyright 2011-2020  Frédéric Sheedy  (email : sheedf@gmail.com)
 *
...
define( 'ETSY_SHOP_VERSION',  '2.3.2');
define( 'ETSY_SHOP_CAHE_PREFIX', 'etsy_shop_cache_');
...
function etsy_shop_process() {
    ...
    if ( $shop_id != '' && $section_id != '' ) {
        ...
        $listings = etsy_shop_getShopSectionListings( $shop_id, $section_id, $language, $limit, $offset );
        if ( get_option( 'etsy_shop_debug_mode' ) && current_user_can( 'edit_posts' ) ) {
            echo( '<h2>' . __( 'Etsy Shop Debug Mode', 'etsy-shop' ) . '</h2>' );
            echo( '<h3>--- ' . __( 'Etsy Debug Mode - version', 'etsy-shop' ) .' ' . ETSY_SHOP_VERSION . ' ---</h3>' );
            echo( '<p>' . __( 'Go to Etsy Shop Options page if you want to disable debug output.', 'etsy-shop' ) . '</p>' );
            print_r( $listings );
        } else {
            if ( !is_wp_error( $listings ) ) {
                ...
                if ( current_user_can( 'edit_posts' ) ) {
                    $name = ETSY_SHOP_CAHE_PREFIX.$shop_id.'-'.$section_id.'-'.$language.'-'.$limit.'-'.$offset;
                    $url = admin_url( 'options-general.php?page=etsy-shop.php&delete='.$name );
                    ...
                    $data .= '<a id="etsy-shop-reset-button" style="" href="'.$url.'" target="_blank">Etsy-Shop: '. __( 'Delete cached content for this part', 'etsy-shop' ) .'</a></div>' ;
                }
...
function etsy_shop_getShopSectionListings( $etsy_shop_id, $etsy_section_id, $language, $limit, $offset ) {
    $name = ETSY_SHOP_CAHE_PREFIX.$etsy_shop_id.'-'.$etsy_section_id.'-'.$language.'-'.$limit.'-'.$offset;
    $etsy_cached_content = get_transient( $name );
    
    if ( false === $etsy_cached_content ) {
        // if language set
        if ( $language != null ) {
            $reponse = etsy_shop_api_request( "shops/$etsy_shop_id/sections/$etsy_section_id/listings/active", '&limit='.$limit.'&offset='.$offset.'&includes=Images&language='.$language );
        } else {
            $reponse = etsy_shop_api_request( "shops/$etsy_shop_id/sections/$etsy_section_id/listings/active", '&limit='.$limit.'&offset='.$offset.'&includes=Images' );
        }
    ...
    if ( get_option( 'etsy_shop_debug_mode' )  && current_user_can( 'edit_posts' ) ) {
        ...
        print_r( '<h3>--- ' . __( 'Etsy Cached Content:', 'etsy-shop' ) .' '. $name . ' ---</h3>' );
        print_r( $etsy_cached_content );
    }
...
function etsy_shop_api_request( $etsy_request, $args = NULL, $noDebug = NULL ) {
    ...
    /*if ( get_option( 'etsy_shop_debug_mode' ) AND !$noDebug ) {
        $request_body  = '<h3>--- Etsy Request URL ---</h3>';
        $request_body .= $url;
        $request_body .= '<h3>--- Etsy Response ---</h3>';
...
function etsy_shop_options_page() {
    ...
    if ( isset( $_GET['delete'] ) ) {
        // REGEX for security!
        $name = str_replace( '/[^a-zA-Z0-9-_]/', '', $_GET['delete'] );

        // make sure is a etsy-shop cache
        if ( strpos( $name, ETSY_SHOP_CAHE_PREFIX ) === 0 ) {
    ...
    if ( $deleted ) {
        echo '<div class="updated fade"><p><strong>'. __( 'Cached content deleted:', 'etsy-shop' ) . ' ' . $deleted_content . '</strong></p></div>';
    }
