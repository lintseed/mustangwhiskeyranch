...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' getting SyncSettings instance action=' . current_action());
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' registering "sync"');
		wp_register_script('sync', WPSiteSyncContent::get_asset('js/sync.js'), array('jquery'),
			WPSiteSyncContent::PLUGIN_VERSION, TRUE);
		wp_register_script('sync-settings', WPSiteSyncContent::get_asset('js/settings.js'),
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' enqueueing "sync"');
		...
		// allow extensions to modify the data
		$option_data = apply_filters('spectrom_sync_option_data', $option_data);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' syncdata=' . var_export($option_data, TRUE));
		wp_localize_script('sync-common', 'syncdata', $option_data);
		wp_enqueue_script('sync-common');
	}
...
//			echo '<span class="screen-reader-text">Dismiss this notice.</span></button>';
		...
		echo '<div id="sync-success-msg">', __('Content successfully sent to Target site.', 'wpsitesynccontent'), '</div>';
		if ($pull_disabled)
			echo '<div id="sync-pull-msg"><div style="color: #0085ba;">', __('Please activate the Pull extension.', 'wpsitesynccontent'), '</div></div>';
		echo '<div id="sync-runtime-err-msg">', __('A PHP runtime error occurred while processing your request. Examine log files for more information.', 'wpsitesynccontent'), '</div>';
		echo '<div id="sync-server-err-msg">', __('An HTTP %err% error occured in sending the request to the Source site.', 'wpsitesynccontent'), '</div>';
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sync data: ' . SyncDebug::arr_sanitize($sync_data));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' - returned object: ' . SyncDebug::arr_sanitize($response));

				// examine API response to see if Pull is running on Target
				$pull_active = TRUE;
				if (isset($response->response)) {
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' - result data: ' . SyncDebug::arr_sanitize($response->response));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' - response data: ' . SyncDebug::arr_sanitize($response_data));
