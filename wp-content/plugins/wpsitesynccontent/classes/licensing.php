...
class SyncLicensing
{
	...
	public function _call_api($endpoint, $params = NULL, $mode = self::MODE_GET)
	{
		...
		$lic = file_exists(dirname(__DIR__) . '/license.tmp');
...
//SyncDebug::log(__METHOD__.'():' . __LINE__, TRUE);
		if (file_exists(dirname(__DIR__) . '/license.tmp'))
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sending ' . var_export($api_params, TRUE));
...
#		$license_data = json_decode(wp_remote_retrieve_body($response));
		$license_data = $this->_license_data;
		if (isset($license_data['new_version']))
			return $license_data['new_version'];
