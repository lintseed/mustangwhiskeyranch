...
// TODO: add setting for "Show Dashboard Widget"
...
class SyncSettings extends SyncInput
{
	...
	// TODO: update name
	const SETTINGS_PAGE = 'sync';

	private function __construct()
	{
		add_action('admin_menu', array($this, 'add_configuration_page'));
		add_action('current_screen', array($this, 'settings_api_init'));
...
//SyncDebug::log(__METHOD__.'() tab=' . $this->_tab);
		...
		echo '<img src="', esc_url(plugin_dir_url(__DIR__) . 'assets/imgs/wpsitesync-logo-blue.png') . '" class="sync-settings-logo" width="97" height="35" />';
...
//SyncDebug::log(__METHOD__.'() returning');
	...
	{
SyncDebug::log(__METHOD__.'() tab=' . $this->_tab . ' action=' . current_action());
		...
		add_settings_field(
			...
			array(											// args
				...
				'size' => '50',
				'disabled' => defined('WPSITESYNC_TARGET'),	// if using constant, don't allow editing
		...
		add_settings_field(
			...
			array(											// args
				...
				'size' => '50',
				'value' => $data['username'],
				'disabled' => defined('WPSITESYNC_USERNAME'),	// if using constant, don't allow editing
		...
		add_settings_field(
			...
			array(											// args
				...
				'disabled' => defined('WPSITESYNC_PASSWORD'),	// if using constant, don't allow editing
		...
		case 'title-slug':	$desc = __('Title, then Slug - Search for matching Content on Target by Post Title, then by Slug.', 'wpsitesynccontent');
			break;
		case 'title':
		default:			$desc = __('Post Title - Search for matching Content on Target by Post Title only.', 'wpsitesynccontent');
			break;
		}
...
//				'description' => __('Optionally removes traces of WPSiteSync for Content on plugin deactivation.', 'wpsitesynccontent'),
		...
		do_action('spectrom_sync_register_settings', $data, $this);
	}
...
//			$title = ' title="' . esc_attr($args['title']) . '" ';
		$click = '';
		if (isset($args['click']))
			$click = ' onclick="' . esc_attr($args['click']) . '" ';

		echo '<button type="button" id="spectrom-button-', $args['name'], '" class="button-primary spectrom-ui-button"',
			$click, '>', $args['title'], '</button>';
	...
	{
		// TODO: remove this and add a type['password'] to the render_input_field() method's arguments
		$attrib = '';
		if (isset($args['size']))
			$attrib = ' size="' . esc_attr($args['size']) . '" ';
		if (isset($args['disabled']) && $args['disabled'])
...
//		echo '<i id="connect-success-indicator" class="fa ', ($args['auth'] ? 'fa-check' : 'fa-close'), '"';
		echo '<i id="connect-success-indicator" class="dashicons ', $icon[$args['auth']], ' auth', $args['auth'], '"';
		echo ' title="';
		if ($args['auth'])
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' clearing host, username, password values=' . SyncDebug::arr_dump($values));
		}
...
//						$re_auth = TRUE;
		...
		// if defines are present and Source is not currently authenticated, re-authenticate #239
		if (0 === SyncOptions::get_int('auth', 0) && defined('WPSITESYNC_PASSWORD')) {
			$out['password'] = WPSITESYNC_PASSWORD;
			// use other defines to override value that will be sent with 'authenticat' API request
			if (defined('WPSITESYNC_TARGET'))
				$out['host'] = WPSITESYNC_TARGET;
			if (defined('WPSITESYNC_USERNAME'))
				$out['username'] = WPSITESYNC_USERNAME;
			$re_auth = TRUE;			// signal re-authentication to be performed
		}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' authenticating...');
			$res = $api->api('auth', $auth);
			if (!is_wp_error($res)) {
				if (isset($res->response->success) && $res->response->success) {
					...
					if (0 === $res->error_code)
						$msg .= ' ' . __('If you\'re using Two-Factor Authentication (2FA) on the Target site, you could try disabling that. Once authenticated, you can enable 2FA again.', 'wpsitesynccontent');
			...
			// if using defines, clear target reference so if they're undefined in the future,
			// there will not be a target site declared and we won't think we're authenticated #239
			if (defined('WPSITESYNC_TARGET'))
				unset($out['host']);
		}
...
//				'<p>' . __('<strong>Authentication Salt:</strong>: Enter a salt to use when Content is sent to current site or leave blank.', 'wpsitesynccontent') . '</p>' .
	...
	public function footer_content($footer_text)
	{
		$rate_text = sprintf(__('Thank you for using <a href="%1$s" target="_blank">WPSiteSync for Content</a>! Please <a href="%2$s" target="_blank">rate us</a> on <a href="%2$s" target="_blank">WordPress.org</a>', 'wpsitesynccontent'),
            ...
            esc_url('https://wordpress.org/support/plugin/wpsitesynccontent/reviews?rate=5#new-post')
		);
