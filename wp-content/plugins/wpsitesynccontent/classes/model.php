...
/**
 * Model for the `spectrom_sync_model` table. This class allows for storing and
 * retrival of Source to Target post ID information.
 */
...
SyncDebug::log(__METHOD__.'() sql=' . str_replace($site_key, substr($site_key, -16), $sql));
		$wpdb->query($sql);
	}

	/**
	 * Saves a sync record to the database.
	 * @param array $data The sync data to be persisted in the database.
	 * @return boolean TRUE on success or FALSE on success.
	 */
	public function save_sync_data($data)
	{
		...
		if (!in_array($data['content_type'], array('comment', 'post', 'postmeta', 'term', 'user')))
			throw new Exception('The `content_type` passed to save_sync_data() is invalid');
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' data=' . SyncDebug::arr_sanitize($data));
		if (NULL !== $sync_data) {
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' read: ' . SyncDebug::arr_sanitize($sync_data));
...
//	SyncDebug::log(__METHOD__.'() sql=' . $wpdb->last_query);
	 ...
	 *	which database the Content identified by $source_id is found in. One of 'post', 'comment', 'term' or 'user'.
	...
	public function get_sync_data($source_id, $site_key = NULL, $type = 'post', $assoc = FALSE)
	{
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
			if (!in_array($type, array('comment', 'post', 'postmeta', 'term', 'user')))
		...
		$join = $where = '';
		if (NULL !== $type) {
			// $type is already sanitized above
			$where = " AND `content_type`='{$type}' ";	// no need to prepare() since it's santized above
		}
...
SyncDebug::log(__METHOD__.'() sql: ' . str_replace($site_key, substr($site_key, -16), $sql));
	...
	public function get_sync_target_data($target_id, $target_site_key = NULL, $type = 'post')
	{
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
			if (!in_array($type, array('comment', 'post', 'postmeta', 'term', 'user')))
				throw new Exception('The `type` passed to get_sync_target_data() is invalid');
		}
...
SyncDebug::log(__METHOD__.'() sql: ' . str_replace($target_site_key, substr($target_site_key, -16), $sql) . ' res=' . var_export($res, TRUE));
		return $res;
	}
	...
	public function get_sync_target_post($source_post_id, $target_site_key, $type = 'post')
	{
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
			if (!in_array($type, array('comment', 'post', 'postmeta', 'term', 'user')))
				throw new Exception('The `type` passed to get_sync_target_post() is invalid');
		}
...
SyncDebug::log(__METHOD__.'() sql=' . str_replace($target_site_key, substr($target_site_key, -16), $sql) . ' returned ' . SyncDebug::arr_sanitize($ret));
		return $ret;
	}
	...
	public function get_source_from_target($target_post_id, $source_site_key, $type = 'post')
	{
		...
		$ret = $wpdb->get_row($sql);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sql=' . str_replace($source_site_key, substr($source_site_key, -16), $sql) . ' returned ' . var_export($ret, TRUE));
		return $ret;
	}
...
SyncDebug::log(__METHOD__.'() post id=' . $post_id); // . ' query: ' . var_export($query, TRUE));

		if (0 === $query->found_posts)
			return $push_data;

		$push_data['post_data'] = (array) $query->posts[0];
		$push_data['post_data']['post_content'] = str_replace('\\u', '~syncescuni~', $push_data['post_data']['post_content']); #259

		// other images connected to the current post ID.
		if (function_exists('get_attached_media'))
			$push_data['post_media'] = get_attached_media('', $post_id);

		// also include the version numbers of WordPress and WPSiteSync
		global $wp_version;
		...
		$push_data['thumbnail'] = get_post_thumbnail_id($post_id);
	...
	private function _build_post_meta($post_id)
	{
		...
		if ($post_meta) {
			...
			foreach ($post_meta as $key => &$value) {
				...
				// check for escaped quotes in JSON data and change to token #257
				foreach ($value as &$meta_entry) {
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key=' . $key . ' value=' . $meta_entry);
					if ($this->is_json($meta_entry))
						$meta_entry = str_replace('\"', '~syncescquote~', $meta_entry);
					$meta_entry = str_replace('\\u', '~syncuni~', $meta_entry);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key=' . $key . ' value=' . $meta_entry);
				}
			}
		} else
			$post_meta = array();	// if it's not an array there was an error. we need to force it to an array
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' meta data=' . var_export($post_meta, TRUE));
		return $post_meta;
	}

	/**
	 * Checks data to see if it includes JSON strings
	 * @param string $str A string suspected of containing JSON data
	 * @return boolean TRUE if string contains JSON encoded data; FALSE if not a string or not containing JSON
	 */
	public function is_json($str)
	{
		...
		if (!empty($str)) {
			...
			$str = str_replace(array('~syncescquote~', '~syncuni~'), array('\\"', '\\u'), $str);
			@json_decode($str);
...
//SyncDebug::log(__METHOD__.'() returning taxonomy information: ' . var_export($tax_data, TRUE));
	 ...
	 * @deprecated Use SyncPostModel::get_post_lock_user()
	 */
	public function get_post_lock_user()
	{
		_deprecated_function(__METHOD__, '1.5', 'SyncPostModel::get_post_lock_user()');
		$ret = array();
