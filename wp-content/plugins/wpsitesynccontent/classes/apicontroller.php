...
class SyncApiController extends SyncInput implements SyncApiHeaders
{
	...
	private $_action = NULL;						// API action being performed
	public $_parent_action = NULL;					// the parent action for the current API call
	private $_sync_model = NULL;					// class property for SyncModel- used in push_complete API calls

	public $source = NULL;							// the URL of the Source site for the request
	public $source_post_id = 0;						// the post id on the Source
	public $post_id = 0;							// the post id being updated on the Target
	public $args = array();
	 ...
	 *	['no_response'] = boolean TRUE to not send SyncApiResponse to requester
...
//SyncDebug::log(__METHOD__.'() args=' . var_export($args, TRUE));
		...
		$action = $this->_action = isset($args['action']) ? $args['action'] : sanitize_key($this->get('action', ''));

		// use response passed as argument if provided
		if (isset($args['response']))
			$response = $args['response'];
		else
			$response = new SyncApiResponse(TRUE, $this->get_header(self::HEADER_SYNC_REQUEST_ID));
		$this->_response = $response;

		if (isset($args['parent_action']))
			WPSiteSyncContent::get_instance()->set_parent_action($args['parent_action']);
		else
			$this->args['parent_action'] = NULL;
		if (isset($args['no_response']) && $args['no_response'])
			$this->args['no_response'] = TRUE;
		else
			$this->args['no_response'] = FALSE;

//		$this->_parent_action = isset($args['parent_action']) ? $args['parent_action'] : NULL;

		if (isset($args['site_key']) || $this->args['no_response']) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' inform SyncApiResponse to NOT send responses');
			$response->nosend = TRUE;		// do not sent API Responses if site_key is provided
		}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' action=' . $action . ' source=' . $this->source . ' key=' . substr($this->source_site_key, -16) . ' request id=' . $this->get_header(self::HEADER_SYNC_REQUEST_ID));
	}

	/**
	 * Dispatches an incoming API action to an appropriate handler function
	 */
...
SyncDebug::log(' sync_nonce=' . $this->get('_spectrom_sync_nonce') . ' site_key=' . substr($this->get('site_key'), -16));
			$this->_response->error_code(SyncApiRequest::ERROR_SESSION_EXPIRED);
			$this->_response->send(NULL === $this->args['parent_action']);		// calls die() if there is a parent action
			return;
		}

		// check authentication for non-'auth' API calls
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' action=' . $this->_action . ' response=' . ($this->args['no_response'] ? 'TRUE' : 'FALSE'));
		if ('auth' !== $this->_action && !$this->args['no_response']) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' checking credentials');
			$auth = new SyncAuth();
			$user = $auth->check_credentials($this->_response);
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' auth failed ' . var_export($user, TRUE));
			if ($this->_response->has_errors()) {
				$this->_response->send(NULL === $this->args['parent_action']);	// calls die() if there is a parent action
				return;			// for Pull requests we want to return and continue processing
			}
		}

		// signal add-ons before processing API request
		do_action('spectrom_sync_before_api', $this->_action);

//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' check action: ' . $action);
		switch ($this->_action) {
		case '':
			$this->_response->error_code(SyncApiRequest::ERROR_UNRECOGNIZED_REQUEST);
			break;

		case 'auth':
			// handles authentication operations
			$auth = new SyncAuth();
			$auth->check_credentials($this->_response);
			break;

		case 'push':
			// handles push operations
			$this->push($this->_response);
			break;

		case 'upload_media':
			// handles media upload operations
			$this->upload_media($this->_response);
			break;

		case 'getinfo':
			$this->get_info($this->_response);
			break;

		case 'push_complete':
			$this->push_complete($this->_response);
			break;

		default:
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' checking for dependent add-ons');
			// TODO: move before switch so this is done for all API requests
			$required = $this->get_header(SyncApiHeaders::HEADER_SYNC_REQUIRED_ADDON);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' header: ' . $required);
			if (!empty($required)) {
				// we know there's a dependency. check that it's active
				$ext = apply_filters('spectrom_sync_active_extensions', array(), FALSE); // get active extensions
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' extensions: ' . var_export($ext, TRUE));
				$found = FALSE;
				...
				if (!$found) {
					$this->_response->error_code(SyncApiRequest::ERROR_EXTENSION_MISSING, $required);
					$this->_response->send();
					return;
				}
			}

SyncDebug::log(__METHOD__."() sending action '{$this->_action}' to filter 'spectrom_sync_api'");
			...
			$res = apply_filters('spectrom_sync_api', FALSE, $this->_action, $this->_response);
			if (FALSE === $res)
				$this->_response->error_code(SyncApiRequest::ERROR_UNRECOGNIZED_REQUEST);
		}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' response has no errors');
			...
			do_action('spectrom_sync_api_process', $this->_action, $this->_response, $this);
		}

//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sending response to Source');
		$this->_response->send(NULL === $this->args['parent_action']);			// calls die() if ther is a parent action
	}

	/**
	 * Callback for SyncApiModel and API initiation (such as Pull operations). Obtains
	 * SyncApiController instance and sends API request to dispatcher.
	 * @param array $args Arguments to send to the SyncApiController constructor
	 */
	public static function api_dispatch($args)
	{
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' retrieving instance...');
		$inst = self::get_instance($args);
if (NULL === $inst)
	SyncDebug::log(__METHOD__.'():' . __LINE__ . ' instance is NULL');
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' dispatching...');
		$inst->dispatch();
		return $inst;
	}
	...
	{
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' ERROR: deprecated', TRUE);
		_deprecated_function(__METHOD__, '1.5.5', 'SyncOptions::has_permission()');

//SyncDebug::log(__METHOD__."('{$cap}')");
		if (NULL === $id) {
//$res = SyncOptions::get_user()->has_cap($cap);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' has_cap(' . $cap . ') returning ' . var_export($res, TRUE));
			return SyncOptions::get_user()->has_cap($cap);
		}
//$res = SyncOptions::get_user()->has_cap($cap, $id);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' has_cap(' . $cap . ') returning ' . var_export($res, TRUE));
		return SyncOptions::get_user()->has_cap($cap, $id);
	}
	...
	{
if (empty($args) && NULL === self::$_instance)
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' ERROR: calling get_instance() with NO ARGS before instance exists', TRUE);
	...
	{
		_deprecated_function(__METHOD__, '1.5.5', 'WPSiteSyncContent->get_parent_action()');
		return WPSiteSyncContent::get_instance()->get_parent_action();
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' read request headers: ' . var_export($this->_headers, TRUE)); ##
		}
...
//SyncDebug::log(' post data: ' . SyncDebug::arr_sanitize($_POST));
//SyncDebug::log(' request data: ' . SyncDebug::arr_sanitize($_REQUEST));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' - post_data=' . SyncDebug::arr_sanitize($post_data, 0x01));
		$post_data['post_content'] = str_replace('~syncescuni~', '\\u', $post_data['post_content']); #259
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' syncing post data Source ID#'. $this->source_post_id . ' - "' . $post_data['post_title'] . '"');
		...
		$post = NULL;
		if (0 !== $target_post_id) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' target post id provided in API: ' . $target_post_id);
			$post = get_post($target_post_id);
		}

		// use Source's post id to lookup Target id
		if (NULL === $post) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' look up target id from source id: ' . $this->source_post_id);
			$model = new SyncModel();
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sync_data: ' . SyncDebug::arr_sanitize($sync_data));
			if (NULL !== $sync_data) {
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found post: ' . var_export($post, TRUE));
...
SyncDebug::log(__METHOD__.'() setting parent post to #' . $parent_post->target_content_id);
		...
		if (0 !== SyncOptions::get_user_id())
			wp_set_current_user(SyncOptions::get_user_id());

		// adjust post_time and post_modified based on Time Zone settings for Source and Target #278
		$source_offset = (float) $this->post_raw('gmt_offset', '');
		$target_offset = (float) get_option('gmt_offset', '');
		// check for empty, not a 0 hour adjustment #278
		if (!empty($this->post_raw('gmt_offset')) && $source_offset !== $target_offset) {
			$post_date_gmt = DateTime::createFromFormat('Y-m-d H:i:s', $post_data['post_date_gmt']);
			$post_date_gmt->add(new DateInterval('PT' . ($target_offset - $source_offset) . 'H'));
			$new_post_date = $post_date_gmt->format('Y-m-d H:i:s');
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' post_date_gmt=' . $post_data['post_date_gmt'] . ' new post_date=' . $new_post_date);
##			$post_data['post_date'] = $new_post_date;
			// same process for post_modified
			$post_modified_gmt = DateTime::createFromFormat('Y-m-d H:i:s', $post_data['post_modified_gmt']);
			$post_modified_gmt->add(new DateInterval('PT' . ($target_offset - $source_offset) . 'H'));
			$new_post_modified = $post_modified_gmt->format('Y-m-d H:i:s');
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' post_modified_gmt=' . $post_data['post_modified_gmt'] . ' new post_modified=' . $new_post_modified);
##			$post_data['post_modified'] = $new_post_date;
		}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' content: ' . SyncDebug::esc_sql($post_data['post_content']));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' created post ID #' . $target_post_id);
			} else {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' user does not have permission to update content');
		...
		// Note: from this point on, we know that we have permission to add/update
		// the content so it's okay to write to the database
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' starting logger');
		$logger->log(
			array(
				...
				'target_user' => SyncOptions::get_user_id(),
				'type' => 'recv',
			)
		);

		$response->set('post_id', $target_post_id);
//SyncDebug::log(__METHOD__.'() adding Target site key ' . substr(SyncOptions::get('site_key'), -16) . ' to response data');
		...
		// handle metadata
		$this->_process_postmeta($target_post_id);
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' push complete, content=' . $temp_post->post_content);
	...
	/**
	 * Handles processing of postmeta data
	 * @param int $post_id The Target's post ID where postmeta data will be written
	 */
	private function _process_postmeta($post_id)
	{
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' handling meta data');
		...
		// initialize this. if needed and NULL, a new SyncSerialize() instance will be instantiated
		$ser = NULL;

		$pmmodel = new SyncPostMetaModel();

		foreach ($post_meta as $meta_key => $meta_value) {
			// handle multiple postmeta entries #287
			$current_entries = $pmmodel->get_post_meta($post_id, $meta_key);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' current postmeta=' . var_export($current_entries, TRUE));

			// find the lower of Source vs. Target postmeta entries for this meta_key
			$entries = min(count($meta_value), count($current_entries));
			// loop through all the entries. adds and deletes are handled after this
			for ($idx = 0; $idx < $entries; ++$idx) { // loop through meta_value array #286
//			foreach ($meta_value as $value) { // loop through meta_value array #286
				$value = $meta_value[$idx];
//$_v = $value;
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key=' . $meta_key . ' (' . gettype($value) . ') value=' . SyncDebug::arr_sanitize($_v));
//$_v = stripslashes($_v);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key=' . $meta_key . ' (' . gettype($value) . ') value=' . SyncDebug::arr_sanitize($_v));
//$_v = maybe_unserialize($_v);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' new value=' . SyncDebug::arr_sanitize($_v));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key start=' . $meta_key . ' value=' . $value);
				$value = stripslashes($value);			// removes slashes added via HTTP POST operation

//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' stripped slashes=' . $value);
				$is_json = $model->is_json($value);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' is_json=' . ($is_json ? 'TRUE' : 'FALSE'));
				// replace token with *double* escaped quote because update_post_meta() calls wp_unslash() #257
				$value = str_replace('~syncescquote~', $is_json ? '\"' : '\\\"', $value);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key before=' . $meta_key . ' value=' . $value);
				if (FALSE !== strpos($value, '~syncuni~')) {
					$value = str_replace('~syncuni~', $is_json ? '\\u' : '\\\\u', $value);		// need to double escape unicode for update_post_meta()
				}
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' key=' . $meta_key . ' value=' . $value);
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' not fixing non-serialized data');
					$value = str_replace($this->source, site_url(), $value);
				}

				// adjust serialized JSON data containing escaped content #275
				if ($is_json) {
					// if it's JSON data, need to add escaping
					// this preserves the escaped data because update_post_meta() calls wp_unslash()
					$value = wp_slash($value);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' add wp_slash() to JSON data ' . var_export($value, TRUE));
				}
...
#					// when the WPSiteSync API is used. This forces initialization so "Call to a member function
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' writing key=' . $meta_key . ' value=' . var_export($value, TRUE));
//				update_post_meta($post_id, $meta_key, maybe_unserialize($value));
				$pmmodel->update_post_meta($current_entries[$idx], $value);		// #287
			} // for $idx < $entries

			if (count($current_entries) > count($meta_value)) {
				// more entries on Target than on Source. Remove extra entries on Target #287
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' removing extra entries');
				for (; $idx < count($current_entries); ++$idx)
					$pmmodel->remove_post_meta($current_entries[$idx]);
			} else if (count($current_entries) < count($meta_value)) {
				// more entries on Source than on Target. Add extra entries on Target #287
				for (; $idx < count($meta_value); ++$idx) {
					// TODO: move postmeta data fixup into a helper method
					$value = $meta_value[$idx];
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' not fixing non-serialized data');
					...
					if ($is_json) {
						// if it's JSON data, need to add escaping
						// this preserves the escaped data because update_post_meta() calls wp_unslash() to remove it
						$value = wp_slash($value);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' add wp_slash() to JSON data ' . var_export($value, TRUE));
					}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' done processing postmeta');
	}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' post=' . SyncDebug::arr_sanitize($_POST));
		...
		// <!-- wp:latest-posts {"categories":{taxonomy-id}}
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found sync data: ' . SyncDebug::arr_sanitize($sync_data));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' cannot find Target ID for Source Content id ' . $source_ref_id);
										$response->error_code(SyncApiRequest::ERROR_GB_ATTACHMENT_ID, $block_name);
									} else {
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' injecting new Gutenberg Cover Block object: "' . $new_obj_data . '" into content');
							...
							case 'wp:image':						// Image Block- resource reference
								$source_ref_id = isset($obj->id) ? abs($obj->id) : 0;
								if (0 !== $source_ref_id) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found reference to post id ' . $source_ref_id);
									// look up Source post ID to see what the Target ID is
									$sync_data = $sync_model->get_sync_data($source_ref_id, $this->source_site_key);
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' source ids=' . implode(',', $source_ids) . ' = ' . var_export($source_ids, TRUE));
								$new_ids = array();					// initialize array of new ids
								foreach ($source_ids as $source_ref_id) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' source ref id=' . var_export($source_ref_id, TRUE));
									// check for NULLs and maintain array size, just skip attachment ID update process #280
									if (NULL === $source_ref_id) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' skipping NULL ref id');
										$new_ids[] = NULL;
									} else {
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' updated gallery ids: ' . var_export($new_ids, TRUE));
								$from = array();
								$to = array();
								if (0 !== count($new_ids)) {
									$obj->ids = $new_ids;
...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' injecting new Gutenberg File Block object: "' . $new_obj_data . '" into content');
							...
							case 'wp:latest-posts':
								$source_cat_id = abs($obj->categories);
								$sync_data = $sync_model->get_sync_data($source_cat_id, $this->source_site_key, 'term');
								if (NULL !== $sync_data) {
									$obj->categories = strval($sync_data->target_content_id);
									$new_obj_data = json_encode($obj);
									$new_obj_len = strlen($new_obj_data);
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' injecting new Gutenberg File Block object: "' . $new_obj_data . '" into content');
									$content = substr($content, 0, $start) . $new_obj_data . substr($content, $end + 1);
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unrecognized block type "' . $block_name . '" - sending through filter');
								...
								if ($content !== $new_content) {		// check to see if add-ons made any modifications
									// calculate new length of json object based on difference between old and new content
									$new_obj_len = strlen($json) + (strlen($new_content) - strlen($content));	// https://github.com/ServerPress/wpsitesync/pull/10
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' replacement content:' . PHP_EOL . $content);
	 ...
	 * @param SyncApiResponse $response API Response object
	 */
	private function _process_shortcodes(SyncApiResponse $apiresponse)
	{
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' POST=' . SyncDebug::arr_sanitize($_POST));
/*
 ...
 */
		$source_post_id = $this->post_int('post_id', 0);
		$sync_model = new SyncModel();
		$sync_data = $sync_model->get_sync_data($source_post_id, $this->source_site_key, 'post');
		if (NULL === $sync_data) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' cannot find Target post from Source ID=' . $source_post_id);
			$apiresponse->error_code(SyncApiRequest::ERROR_POST_CONTENT_NOT_FOUND);
			return;
		}
		$target_post_id = abs($sync_data->target_content_id);
		$target_post = get_post($target_post_id);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' post ID ' . $target_post_id . ' has ' . strlen($target_post->post_content) . ' bytes of content');
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' content: ' . $target_post->post_content);
		$content = $target_post->post_content;
		$updated = 0;										// count how many shortcodes were updated

		// construct a list of all shortcodes that need updating
		$shortcodes = SyncShortcodes::get_shortcodes();
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' working with shortcodes: ' . var_export($shortcodes, TRUE));
		$known_shortcodes = array_keys($shortcodes);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' known shortcodes: ' . var_export($known_shortcodes, TRUE));
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' matches=' . var_export($matches, TRUE));
		if ($num > 0 && array_key_exists(2, $matches)) {
			...
			$idx = 0;
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found ' . $num . ' shortcodes');
			foreach ($matches[2] as $shortcode) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' updating shortcode attributes for: ' . $matches[0][$idx]);
				$sce = new SyncShortcodeEntry($shortcode, $matches[0][$idx], $matches[3][$idx]);

				// update all attributes found within the shortcode
				foreach ($sce->parse_attributes($shortcodes[$shortcode]) as $type_name => $type_code) {
					// only process if shortcode contains reference to the current attribute
					if ($sce->has_attribute($type_name)) {
						switch ($type_code) {
						case SyncShortcodeEntry::TYPE_IMAGE_ID:				// image ID, all children included
						case SyncShortcodeEntry::TYPE_POST_ID:				// post ID, all children are included in gallery
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' attribute=[' . $type_name . '] type=' . $type_code);
							$ids = trim($ids, '"\'');
							$id_list = explode(',', $ids);
							$target_ids = array();
							foreach ($id_list as $post_id) {
								$post_id = abs($post_id);
								$sync_data = $sync_model->get_sync_data($post_id, $this->source_site_key);
								if (NULL !== $sync_data) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found post reference: ' . $post_id . ' and converting to ' . $sync_data->target_content_id);
									$target_ids[] = abs($sync_data->target_content_id);
								} else {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unable to find Source ID=' . $post_id);
									// TODO: error
								}
							}
							// all IDs have been updated for this attribute, time to rebuild the attribute
							$sce->set_attribute($type_name, implode(',', $target_ids));
							break;

						case SyncShortcodeEntry::TYPE_POST_ATTACH:
							$id = abs($sce->get_attribute($type_name));
							$sync_data = $sync_model->get_sync_data($id, $this->source_site_key);
							if (NULL !== $sync_data) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found post attachment reference: ' . $id . ' and converting to ' . $sync_data->target_content_id);
								$sce->set_attribute($type_name, $sync_data->target_content_id);
							}
							break;

						case SyncShortcodeEntry::TYPE_ATTACHMENT:
							$post_id = $sce->get_attachment_id($value = $sce->get_attribute($type_name));
							if (0 !== $post_id) {
								$sync_data = $sync_model->get_sync_data($post_id, $this->source_site_key);
								if (NULL !== $sync_data) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found post reference ' . $post_id . ' and converting to ' . $sync_data->target_content_id);
									$sce->set_attribute($type_name, str_replace(strval($post_id), strval($sync_data->target_content_id), $value));
								} else {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unable to find Source ID=' . $post_id);
									// TODO: error
								}
						...
						case SyncShortcodeEntry::TYPE_TAXONOMY:
							$tax_id = abs($sce->get_attribute($type_name));
							$sync_data = $sync_model->get_sync_data($tax_id, $this->source_site_key, 'term');
							if (NULL !== $sync_data) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' found taxonomy reference ' . $tax_id . ' and converting to ' . $sync_data->target_content_id);
								$sce->set_attribute($type_name, $sync_data->target_content_id);
							} else {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unable to find Source Taxonomy ID ' . $tax_id);
								// TODO: error
							}
							break;
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unrecognized attribute type code: ' . $type_code);
				...
				// now that attributes are updated, rebuild the shortcode
				$new_shortcode = $sce->__toString();
				if ($sce->get_original_shortcode() !== $new_shortcode) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' updating shortcode content orig=|' . $sce->get_original_shortcode() . '| change to=|' . $new_shortcode . '|');
					$content = str_replace($sce->get_original_shortcode(), $new_shortcode, $content);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' content: ' . $content);
		...
		if (0 !== $updated) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' ' . $updated. ' shortcode changes were made to the content, updating');
			$post_data = array(
				'ID' => $target_post_id,
				'post_content' => $content,
			);
			$wp_error = wp_update_post($post_data, TRUE);
			if (is_wp_error($wp_error)) {
				$apiresponse->error_code(SyncApiRequest::ERROR_CONTENT_UPDATE_FAILED);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' content update failed for ID=' . $target_post_id);
			}
		}
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' completed processing of shortcode entries');
	}

	/**
	 * Used by gutenberg_modify_block_contents() method. Similar to str_replace() but performs a single pass through the $subject string to avoid multiple replacements of search strings.
...
//SyncDebug::log(__METHOD__.'() ** removing term #' . $post_term->term_id . ' ' . $post_term->slug . ' [' . $post_term->taxonomy . ']');
				...
				// if the $post_term is present we don't need to do anything
				$found = TRUE;
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' max upload size=' . wp_max_upload_size() . ' file size=' . (isset($_FILES['sync_file_upload']['size']) ? $_FILES['sync_file_upload']['size'] : '-'));
		...
		if (!SyncOptions::has_permission('upload_files')) {
			$response->error_code(SyncApiRequest::ERROR_NO_PERMISSION);
			$response->send();
		}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' FILES=' . SyncDebug::arr_sanitize($_FILES));
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' POST=' . SyncDebug::arr_sanitize($_POST));
...
//			$response->notice_code(SyncApiRequest::NOTICE_FILE_EXISTS);
			...
			$attach_data = array(
				'post_title' => $this->post('attach_title', ''),
				'post_name' => $this->post('attach_name', ''),
				'post_content' => $this->post('attach_desc', ''),			// '',
				'post_excerpt' => $this->post('attach_caption', ''),
			);
			global $wpdb;
			$wpdb->update($wpdb->posts, $attach_data, array('ID' => $attachment_id));
			update_post_meta($attachment_id, '_wp_attachment_image_alt', $this->post('attach_alt', ''));
...
/*			$media_data = array(
			...
			$source_attach_id = abs($this->post_int('attach_id'));
			if (0 !== $source_attach_id) {							// don't save for attachments not in Media Library #250
				// save to the sync table for later reference
				$sync_data = array(
					'site_key' => $this->source_site_key,
					'source_content_id' => $source_attach_id,		// abs($this->post_int('attach_id')),
					'target_content_id' => $attachment_id,
					'content_type' => $content_type,
					'target_site_key' => SyncOptions::get('site_key'),
				);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' save reference for source media id=' . $source_attach_id . ' target media id=' . $attachment_id);
	...
	/**
	 * Handles the 'push_complete' API calls on the Target
	 * @param SyncApiResponse $response The response instance
	 */
	public function push_complete(SyncApiResponse $response)
	{
SyncDebug::log(__METHOD__.'():' . __LINE__);
		// TODO: update _process_gutenberg() and _process_shortcodes() to use _sync_model property
		$this->_sync_model = new SyncModel();

		$this->_process_gutenberg($response);
		$this->_process_shortcodes($response);	#102

		$target_post_id = 0;
		$source_post_id = $this->post_int('post_id');
		$sync_data = $this->_sync_model->get_sync_data($source_post_id, $this->source_site_key);
		if (NULL !== $sync_data) {
			$target_post_id = abs($sync_data->target_content_id);
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' Source post ID #' . $source_post_id . ' to Target post ID #' . $target_post_id);

			// content is fixed up, now adjust list of post children
			$source_child_ids = $this->post_raw('children');
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' processing child ids: ' . var_export($source_child_ids, TRUE));
			if (is_array($source_child_ids) && 0 !== count($source_child_ids)) {
				$original_children = get_children(array('post_parent' => $target_post_id), OBJECT);		// used to remove children
				$target_child_ids = array();									// used to track list of target child IDs

				foreach ($source_child_ids as $source_attachment_id) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' updating attachment ID #' . $source_attachment_id);
					$sync_data = $this->_sync_model->get_sync_data($source_attachment_id, $this->source_site_key);
					if (NULL !== $sync_data) {
						$target_attachment_id = abs($sync_data->target_content_id);
						$target_child_ids[] = $target_attachment_id;
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' setting id #' . $target_attachment_id . ' post_parent=' . $target_post_id);
						$post_data = array(
						...
						$wp_error = wp_update_post($post_data, TRUE);
						if (is_wp_error($wp_error)) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' error updating post: ' . var_export($wp_error, TRUE));
						}
					} else {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' unable to find target post ID from source ID #' . $source_attachment_id);
					}
				}

				// post_parent columns updated, now update any that need removal
				foreach ($original_children as $attachment) {
					$attachment_id = abs($attachment->ID);
					if (!in_array($attachment_id, $target_child_ids)) {
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' attachment #' . $attachment_id . ' no longer child of post #' . $this->post_id);
						$post_data = array(
							'ID' => $attachment_id,
							'post_parent' => 0,
						);
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' error updating post: ' . var_export($wp_error, TRUE));
				...
				} // foreach ($original_children)
			} // is_array
		} else { // null !== $sync_data
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' cannot find Target post from Source ID #' . $source_post_id);
		}

		// signal add-ons that Push is complete and all Source IDs have been updated to Target IDs
		do_action('spectrom_sync_push_complete', $source_post_id, $target_post_id, $response);
	}
