...
//			$values[] = '';
	...
	private function _maintenance_check()
	{
		...
		// look for 'Comming Soon' v6+ plugin
		if (defined('SEEDPROD_VERSION')) {
			add_filter('option_seedprod_settings', array($this, 'filter_coming_soon_6_setting'), 10, 1);
			return;
		}
		// disable the Membership plugin's redirect to login for "Private Sites" #281
		add_filter('members_is_private_blog', '__return_false', 100);
...
//die('inside ' . __METHOD__. '():' . __LINE__ . ' set=' . var_export($settings, TRUE));
	...
	/**
	 * Filter for the Coming Soon plugin's configuration settings. Used to turn off it's features during API requests.
	...
	/**
	 * Filter for Comming Soon v6+ plugin's configuration settings. Used to turn off it's features during API requests.
	 * @param string $option The Coming Soon plugin's settings
	 * @return string The modified settings, with the 'enable_coming_soon_mode' turned off.
	 */
	public function filter_coming_soon_6_setting($option)
	{
		return '{"enable_maintenance_mode":false,"enable_coming_soon_mode":false}';
	}
