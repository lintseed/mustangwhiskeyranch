...
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' op=' . var_export($operation, TRUE));
		...
		if (sanitize_key($operation) !== $operation) {
			$response->error_code(SyncApiRequest::ERROR_UNRECOGNIZED_OPERATION, $operation);
			$response->send();
		}
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' op=' . var_export($operation, TRUE));

		// perform authentication checking: must be logged in
		if (!is_user_logged_in()) {
			$response->error_code(SyncApiRequest::ERROR_SESSION_EXPIRED, $operation);
			$response->send();
		}
		// must have capability, an 'Author' role or higher
		if (!current_user_can('publish_posts')) {
			$response->error_code(SyncApiRequest::ERROR_NO_PERMISSION, $operation);
			$response->send();
		}

SyncDebug::log(__METHOD__.'():' . __LINE__ . ' signaling api readiness');
		// signal add-ons that user is initialized and API hooks can be set up
		do_action('spectrom_sync_api_init');

		// TODO: move nonce check here to perform test earlier
		if (SyncOptions::has_cap()) {	// has_cap() checks capabilities based on configured Roles
			...
			case 'verify_connection':
				...
				break;
			default:
//SyncDebug::log(__METHOD__.'():' . __LINE__ . ' filter "spectrom_sync_ajax_operation" op=' . var_export($operation, TRUE));
...
//SyncDebug::log(__METHOD__.'() operation "' . $operation . '" returning ' . var_export($response, TRUE));
	...
	public function verify_connection(SyncApiResponse $response)
	{
		...
		if (is_wp_error($e)) {
			...
			$response->error_code(SyncApiRequest::ERROR_BAD_CREDENTIALS, $e->get_error_message());
		} else
			$response->success(TRUE);
	...
	public function push(SyncApiResponse &$response)
	{
		...
		if ($api_response->is_success()) {
			$response->success(TRUE);
			$response->set('message', __('Content successfully sent to Target site.', 'wpsitesynccontent'));
		} else {
